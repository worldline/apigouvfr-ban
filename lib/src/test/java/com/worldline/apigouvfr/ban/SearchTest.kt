...
public class SearchTest {
    ...
    private inline fun <reified T : Any> mockHttpClient(httpStatusCode: HttpStatusCode, response: T? = null) {
        banRepositoryImpl = BanRepositoryImpl(coroutineDispatcher, apiMockEngine(httpStatusCode, response))
    }
    ...
    @Test
    public fun `search when parameters ok then Success`(): TestResult = runTest(coroutineDispatcher) {
        // Given
        mockHttpClient(HttpStatusCode.OK, defaultBanFeatureCollectionResponse)

        // When
        val result = banRepositoryImpl.search(BanSearchParameters(query = "test"))
    ...
    @Test
    public fun `search when HTTP 404 then ServerError`(): TestResult = runTest(coroutineDispatcher) {
        // Given
    ...
    @Test
    public fun `search when client error then ClientError`(): TestResult = runTest(coroutineDispatcher) {
        // Given
